...
def calculate_metrics(logits, labels, num_classes, n_bins=15):
    ...
    ece_metric = CalibrationError(
        n_bins=n_bins, norm="l1", task=task_type, num_classes=num_classes
    )
    mce_metric = CalibrationError(
        n_bins=n_bins, norm="max", task=task_type, num_classes=num_classes
    )
...
def generate_results(all_scores, all_labels, noise, i, paths):
    ...
    print(
        f"results for model with {noise} noise: f1-score: {f1}, precision: {precision}, recall: {recall}"
    )
...
def block_diff(model, model_new):
    ...
    for block_name in blocks_to_compare:
        ...
        param_names = []
        param_diff_f = []
        for (pretrained_param_name, pretrained_param), (
            fine_tuned_param_name,
            fine_tuned_param,
        ) in zip(
            pretrained_block.named_parameters(), fine_tuned_block.named_parameters()
        ):
...
def generate_equal_subsets(dataset, num_splits):
    ...
    splitter = StratifiedShuffleSplit(
        n_splits=num_splits, test_size=None, random_state=42
    )
